...
% Parametres
lambda = 0.04;			% Poids de la regularisation       
...
subplot(1,4,1)
	imagesc(max(0,min(1,reshape(u0,nb_lignes,nb_colonnes))),[0 1])
	colormap gray
	axis image off
	title('Image bruitee','FontSize',20)   
subplot(1,4,2)
p2=	imagesc(max(0,min(1,reshape(u0,nb_lignes,nb_colonnes))),[0 1]);
	axis image off
	title('Image restauree','FontSize',20) 
	p2.CData = max(0,min(1,u0));
subplot(1,4,3)
p3 = imagesc(max(0,min(1,reshape(u0 - u_k,nb_lignes,nb_colonnes))),[0 1]);
    axis image off
	title('Image Difference','FontSize',20) 
	p3.CData = max(0,min(1,u0));
subplot(1,4,4)
	p4 = semilogy(tab_t,tab_convergence); 
	axis tight
	...
	p4.YDataSource = 'tab_convergence';
drawnow 
...
while convergence > critere_arret_convergence
	...
	u_kp1 = pas_variation_totale(u0,u_k,lambda,Dx,Dy,epsilon,critere_arret_convergence);
	...
	set(p2, 'CData', reshape(u_k,[nb_lignes nb_colonnes]));
	set(p3, 'CData', reshape(u0 - u_k,[nb_lignes nb_colonnes]));
